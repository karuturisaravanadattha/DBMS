...
SELECT AVG(salary) from employee WHERE department = 'Testing' and gender = 'Female';

SELECT count(*) FROM employee WHERE department = 'SW' AND gender = 'Male';
 
SELECT  name FROM employee WHERE salary > 75000 order by age ASC , name;
...
SELECT name from employee WHERE date_of_joining = (select min(date_of_joining) from employee);
...
select avg(salary) from employee where department = 'Testing' and date_of_joining > '02-09-2019';
select count(*) from employee where department ='Testing' and salary < 45000;
...
GROUP BY gender;
--GROUP BY 
-- all the columns used in e select statement
...
select gender, max(age), min(age),min(date_of_joining)
FROM employee
GROUP BY gender;
