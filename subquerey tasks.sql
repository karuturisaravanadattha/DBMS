...
WHERE DATE_OF_JOINING = (SELECT min(DATE_OF_JOINING) FROM employee);
 ...
 1.1.3
 Table: employee
 Find out who joined the organization very recently, and print their id, name, joining date
 Note: If more than one employee falls under this category, you must show the details of all such
 employee
 SELECT id,name,DATE_OF_JOINING FROM employee
...
WHERE salary = (SELECT MAX(salary) FROM employee WHERE department= 'Testing');
...
WHERE salary > (SELECT avg(salary) FROM employee where department='SW')
AND department='HR';
...
1.1.8
 ...
 employees
SELECT name FROM employee
WHERE salary > (SELECT max(salary) FROM employee where department='Testing' AND gender= 'Male')
 AND gender = 'Female' AND department='Operations';
